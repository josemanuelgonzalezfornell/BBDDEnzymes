...
createfileinitialbutton=Button(mainmenu, text="Crear Archivo", width=15, command=lambda:createBBDD())
createfileinitialbutton.grid(column=0, row=0, pady=2)

conectinitialbutton=Button(mainmenu, text="Conectar BBDD", width=15, command=lambda:connectToBBDD())
...
modifyinitialbutton=Button(mainmenu, text="Modificar", width=15, command=lambda:openModifyWindow(root))
...
actionmenu.add_command(label="Modificar", command=lambda:openModifyWindow(root))
