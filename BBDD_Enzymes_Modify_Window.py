...
def openModifyWindow(root):
    while True:
        try:
            ...
            puntoisoelectricolabel = Label(
                textframe, text="Punto isoelectrico", padx=10)
            puntoisoelectricolabel.grid(row=5, column=4, sticky=W)
            puntoisoelectricotext = Entry(
                textframe, textvariable=puntoisoelectrico)
            ...
            # Botones
            searchbutton = Button(textframe, text="Buscar", width=5, command=lambda: searchId(id, name, microrganism, type, vector, ph, activity, substrate, adjustmenttype, intraextracellular, soluble, protocolpurification, protocolreplegament,
                                  aa, mw, temperatureexpression, temperatureactivity, puntoisoelectrico, iptg, coefficient, timeexpression, km, kmunit, kmerror, kcat, kcatunit, kcaterror, kmkcat, kmkcatunit, kmkcaterror, seqdnatext, seqaatext, commentstext))
            searchbutton.grid(row=1, column=2)
            modifybutton = Button(bottonframe, text="Modificar", width=15, command=lambda: ModifyData(id.get(), name.get(), microrganism.get(), type.get(), vector.get(), ph.get(), activity.get(), substrate.get(), adjustmenttype.get(), intraextracellular.get(), soluble.get(), protocolpurification.get(), protocolreplegament.get(), aa.get(), mw.get(), temperatureexpression.get(
            ), temperatureactivity.get(), puntoisoelectrico.get(), iptg.get(), coefficient.get(), timeexpression.get(), km.get(), kmunit.get(), kmerror.get(), kcat.get(), kcatunit.get(), kcaterror.get(), kmkcat.get(), kmkcatunit.get(), kmkcaterror.get(), seqdnatext.get(1.0, END).rstrip("\n"), seqaatext.get(1.0, END).rstrip("\n"), commentstext.get("1.0", END).rstrip("\n")))
            modifybutton.grid(row=0, column=0, padx=2)
            erasedatabutton = Button(
                bottonframe, text="Borrar campos", width=15, command=lambda: EraseData())
            erasedatabutton.grid(row=0, column=1, padx=2)
            eraseinstance = Button(bottonframe, text="Borrar instancia", width=15, command=lambda: DeleteData(id, name, microrganism, type, vector, ph, activity, substrate, adjustmenttype, intraextracellular, soluble, protocolpurification, protocolreplegament,
                                   aa, mw, temperatureexpression, temperatureactivity, puntoisoelectrico, iptg, coefficient, timeexpression, km, kmunit, kmerror, kcat, kcatunit, kcaterror, kmkcat, kmkcatunit, kmkcaterror, seqdnatext, seqaatext, commentstext))
            ...
            actionmenu.add_command(label="Modificar", command=lambda: ModifyData(id.get(), name.get(), microrganism.get(), type.get(), vector.get(), ph.get(), activity.get(), substrate.get(), adjustmenttype.get(), intraextracellular.get(), soluble.get(), protocolpurification.get(), protocolreplegament.get(), aa.get(), mw.get(), temperatureexpression.get(
            ), temperatureactivity.get(), puntoisoelectrico.get(), iptg.get(), coefficient.get(), timeexpression.get(), km.get(), kmunit.get(), kmerror.get(), kcat.get(), kcatunit.get(), kcaterror.get(), kmkcat.get(), kmkcatunit.get(), kmkcaterror.get(), seqdnatext.get(1.0, END).rstrip("\n"), seqaatext.get(1.0, END).rstrip("\n"), commentstext.get("1.0", END).rstrip("\n")))
            actionmenu.add_command(label="Borrar campos",
                                   command=lambda: EraseData())
            actionmenu.add_command(label="Borrar instancia", command=lambda: DeleteData(id, name, microrganism, type, vector, ph, activity, substrate, adjustmenttype, intraextracellular, soluble, protocolpurification, protocolreplegament,
                                   aa, mw, temperatureexpression, temperatureactivity, puntoisoelectrico, iptg, coefficient, timeexpression, km, kmunit, kmerror, kcat, kcatunit, kcaterror, kmkcat, kmkcatunit, kmkcaterror, seqdnatext, seqaatext, commentstext))
